function findcalls_v5_HaHo(wd,fs,varargin)
...
%for call eval
durthresh=30; %in ms : postmerge is the call long enough (3 for trills)
mergethresh = 20e-3; % in s : any windows less than mergethresh (5ms) apart will be combined (5e-3 for chirps 20e-3 for trills)
rmsthresh=0.005;%has to be adapted to recording quality : is it loud enough premerge?
...
if adaptiveThreshold 
    ...
    thresh = 0.001; %determines the initial premerge voltage cutoff min
...
if debug
    figure;
    manual_classification = zeros(1,n_files);
    low2high = nan(1,n_files);
end
total_callcount=0;
for fln = 1:n_files
    file_callcount = 0;
